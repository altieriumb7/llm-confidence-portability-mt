...
\begin{abstract}
This paper evaluates whether self-reported confidence is portable across providers for English$\rightarrow$German machine translation under one shared protocol. Eight models (OpenAI, Anthropic, Google) are run at temperature 0 on 500 WMT17 sentences, yielding 4,000 translation--confidence pairs. Quality is scored with sentence-level chrF, and low quality is operationally defined as the within-model bottom 20\% tail. Relative to that target, expected calibration error (ECE) and mismatch@0.9 show large provider differences despite similar mean chrF (54.0--60.1): ECE ranges from 0.076 to 0.180, and mismatch@0.9 ranges from 0.2\% to 18.2\%. The released snapshot also shows uneven parse-warning burden (443/4000 rows; 11.1\%), which is an implementation-level confound for cross-provider comparison. The main conclusion is scoped: under this protocol and chrF-based target, self-reported confidence is not portable as a shared probability scale, and a common threshold (e.g., $\tau=0.9$) does not induce comparable operating points. Supplementary analyses show that provider-specific isotonic remapping can often reduce ECE but does not restore portability. Limitations are explicit: the target is operational rather than semantic, COMET/xCOMET is not bundled offline, the semantic-audit scaffold is included without completed labels in this release, and non-baseline prompt-variant outputs are not bundled.
\end{abstract}
