...
def main():
    ...
    for r in rows:
        ...
        raw_conf = r.get("confidence", r.get("conf"))
        r["conf"] = coerce_confidence(raw_conf)

        toks = tokenize(r.get("src", ""))
        r["src_len_tokens"] = len(toks)
        r["src_len_chars"] = len(r.get("src", ""))
        r["punct_count"] = sum(1 for c in r.get("src", "") if not c.isalnum() and not c.isspace())
