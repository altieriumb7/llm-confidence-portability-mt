...
def main():
    ...
    for m in selected_models:
        ...
        done_ids = {
            ...
            if ("id" in r and (r.get("conf") is not None or r.get("confidence") is not None) and (r.get("hyp") or r.get("translation")))
        }
        ...
        for row in data:
            ...
            try:
                ...
                if provider == "gemini" and ("RESOURCE_EXHAUSTED" in msg or "Quota exceeded" in msg or "limit: 0" in msg):
                    logger.error("Gemini quota exhausted; stopping remaining Gemini requests for this run.")
                    skip_providers.add("gemini")
                    break
