...
AGG_DIR="${AGG_DIR:-runs/aggregated}"
...
"$PYTHON_BIN" src/03_features_and_metrics.py \
  --config "$CONFIG" \
  --input_dir "$RAW_DIR" \
  --output "$AGG_DIR/dataframe.csv"

"$PYTHON_BIN" src/04_analysis_and_plots.py \
  --config "$CONFIG" \
  --input "$AGG_DIR/dataframe.csv" \
...
echo "Regenerated table/source aggregates in $AGG_DIR (no binary plots generated)."
